...
export function outputJson(result) {
    ...
    if (result.changeId !== undefined)
        json.changeId = result.changeId;
    if (result.next !== undefined)
        json.next = result.next;
...
export function outputText(result) {
    ...
    if (result.actionRequired) {
        ...
        if (ar.allowedFiles.length > 0) {
            console.log(`\nAllowed Files:`);
            for (const f of ar.allowedFiles)
                console.log(`  - ${f}`);
        }
        if (ar.expectedNewFiles.length > 0) {
            console.log(`\nExpected New Files:`);
            for (const f of ar.expectedNewFiles)
                console.log(`  - ${f}`);
        }
        if (ar.forbiddenFiles.length > 0) {
            console.log(`\nForbidden Files:`);
            for (const f of ar.forbiddenFiles)
                console.log(`  - ${f}`);
        }
    ...
    if (result.data &&
        ...
        if (Array.isArray(events) && events.length > 0) {
            ...
            for (const e of events) {
                const time = typeof e.createdAt === "string"
    ...
    if (result.data &&
        ...
        const loop = result.data.activeLoop;
    ...
    if (result.error) {
        ...
        if (result.error.next)
            console.error(`  → ${result.error.next}`);
    }
}
