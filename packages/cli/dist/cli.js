...
import { CodebaseAdapter, Core } from "@sdd-harness/core";
import { CodebaseMemoryManager, CodebaseMemoryTransport, } from "@sdd-harness/codebase-memory";
...
async function main() {
    ...
    const extraArgs = {};
    if (values.change)
        extraArgs.changeId = values.change;
    if (values.timeout)
        extraArgs.timeout = Number(values.timeout);
    if (values["non-interactive"])
        extraArgs.nonInteractive = true;
    if (values.force)
        extraArgs.force = true;
    if (values.verbose)
        extraArgs.verbose = true;
    if (values.agent)
        extraArgs.agent = values.agent;
    if (values.structurePolicy)
        extraArgs.structurePolicy = values.structurePolicy;
    if (values.host)
        extraArgs.host = values.host;
    if (values.intent)
        extraArgs.intent = values.intent;
    if (values.answers) {
        try {
    ...
    switch (command) {
        ...
        case "build": {
            ...
            result = await runBuild(core, cwd, subcommand, values.task, values.result, extraArgs, undefined);
            break;
        }
        case "verify":
        ...
        case "auto": {
            if (values.resume)
                extraArgs.resume = values.run ?? true;
            if (values.restart)
                extraArgs.restart = true;
            if (values.stop)
                extraArgs.stop = true;
            if (values.events) {
                extraArgs.events = true;
                if (values.tail)
                    extraArgs.tail = Number(values.tail);
            }
            if (values["loop-status"])
                extraArgs.loopStatus = true;
            const requirement = positionals.slice(1).join(" ") || "";
        ...
        case "codebase": {
            ...
            if (query)
                extraArgs.query = query;
            if (!extraArgs.intent && values.intent)
                extraArgs.intent = values.intent;
