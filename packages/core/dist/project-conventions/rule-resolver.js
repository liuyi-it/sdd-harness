...
export async function resolveProjectRules(root, allowedFiles, host = "codex") {
    ...
    return {
        ...
        hash: `sha256:${sha256(JSON.stringify(sources.map(({ path, scope, sha256, priority }) => ({
            path,
            scope,
            sha256,
            priority,
...
function collectScopes(files) {
    ...
    for (const file of files) {
        ...
        if (normalized === "" || normalized.startsWith(".."))
            continue;
        const directory = normalized.includes(".") && !normalized.endsWith("/")
            ? dirname(normalized)
            : normalized;
        if (directory === ".")
            continue;
        const segments = directory.split("/");
        if (segments.some((segment) => IGNORED_SCOPES.has(segment)))
            continue;
        for (let index = 1; index <= segments.length; index += 1) {
            scopes.add(segments.slice(0, index).join("/"));
        }
    }
    return [...scopes].sort((left, right) => scopeDepth(left) - scopeDepth(right));
}
function rulePriority(host, file) {
    if (host === "codex")
        return file === "AGENTS.md" ? 0 : 1;
