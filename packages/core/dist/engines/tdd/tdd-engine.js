...
export class TddEngine {
    generateDesign(input) {
        const affectedFiles = extractPaths(`${input.impact}\n${input.codebaseSummary}\n${input.architecture}`);
...
function renderTestPlan(requirements) {
    return [
        "# Test Plan",
        ...requirements.flatMap((requirement) => requirement.scenarios.flatMap((scenario) => [
            "",
...
function list(title, values, level = 0) {
...
function structuredRequirementLines(spec) {
    const lines = spec
        .split(/\r?\n/)
...
//# sourceMappingURL=tdd-engine.js.map