 ...
 */
export function canResumeCommand(state, command) {
    return ((state.currentPhase === "FAILED" && state.failedCommand === command) ||
...
export function assertRecoverableCommandState(state, command) {
    if (!canResumeCommand(state, command))
        return;
    ...
    if (state.previousPhase === null ||
        ...
        throw new SddError("E_STATE_CORRUPTED", `恢复命令 ${command} 的状态上下文不合法：previousPhase=${state.previousPhase ?? "null"}，inProgressPhase=${state.inProgressPhase ?? "null"}`, "sdd status");
    }
}
export function previousStablePhase(state, fallback) {
    return state.currentPhase === "FAILED" || state.currentPhase === "PAUSED"
        ? (state.previousPhase ?? fallback)
        : state.currentPhase;
}
export function normalizeCommandError(error, fallbackCode, next) {
    if (error instanceof SddError)
        return error;
    if (isNodeError(error) && error.code === "ENOENT") {
        return new SddError("E_MISSING_ARTIFACT", `缺少必要制品：${error.path ?? error.message}`, next);
    }
    if (error instanceof SyntaxError) {
        return new SddError("E_STATE_CORRUPTED", `状态或制品解析失败：${error.message}`, next);
    }
...
export async function persistCommandFailure(store, error, options) {
