...
import { driftFailures, reviewGate, verifyGate, } from "../quality/quality-gates.js";
import { assertTaskResultIds, parseTaskResults, parseTasks, } from "../quality/quality-schema.js";
import { readAuthoritativeSpec, renderTraceability, traceabilityFailures, } from "../quality/traceability.js";
import { getPolicy, resolvePolicyBundle, } from "@sdd-harness/agent-policies";
/**
 ...
 * - 将状态推进到 ARCHIVED
 */
export async function runArchive(root, args, signal) {
    ...
    let previousPhase = "REVIEW_READY";
    try {
        ...
        if (state.currentPhase !== "REVIEW_READY" &&
            ...
            throw new SddError("E_REVIEW_REQUIRED", `无法在 ${state.currentPhase} 状态下执行 archive`, state.suggestedCommand ?? "sdd review");
        }
        ...
        const { archivedMarker } = await withTimeout((async () => {
            ...
            await assertPassReport(writer, join(change, "review-report.md"), reviewReport, "E_REVIEW_REQUIRED", "sdd review");
            ...
            if (!sameSnapshot(currentSnapshot, verifySnapshot) ||
                ...
                throw new SddError("E_VERIFY_REQUIRED", "当前 Git 快照与 verify/review 快照不一致", "sdd verify");
            const gate = verifyGate(document, tasks, parsedResults, state.tasks);
            const review = reviewGate(tasks, parsedResults);
            const reportedFiles = parsedResults.flatMap((result) => result.modifiedFiles);
            ...
            const artifactFailures = traceabilityFailures(document, tasks, parsedResults, true);
            if (!gate.passed)
                throw new SddError("E_VERIFY_REQUIRED", gate.failures.join("; "), "sdd verify");
            if (!review.passed)
                throw new SddError("E_REVIEW_REQUIRED", review.failures.join("; "), "sdd review");
            if (artifactFailures.length > 0)
                throw new SddError("E_MISSING_ARTIFACT", artifactFailures.join("; "), "sdd archive");
        ...
        const normalized = normalizeCommandError(error, "E_STATE_CORRUPTED", "sdd archive");
...
function collectPolicyRefs(tasks, design) {
    const bundles = [
...
async function convergeArchivedState(store) {
...
async function writeArchiveAudit(root, phase, changeId) {
...
async function hasValidMarker(change, changeId) {
    try {
        const value = JSON.parse(await readFile(join(change, ".archived"), "utf8"));
...
async function assertPassReport(writer, path, report, code, next) {
    ...
    if (!(await writer.isUnmodified(path)) ||
        matches.length !== 1 ||
        matches[0][1] !== "PASS")
        throw new SddError(code, `${path.split("/").at(-1)} 不是可信的唯一 PASS 报告`, next);
}
async function resolveFinalHead(businessRoot, requireGitHead) {
    try {
