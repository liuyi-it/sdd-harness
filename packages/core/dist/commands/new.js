...
export async function runNew(root, rawArgs, engine, codebase, signal) {
    ...
    try {
        ...
        const previewImpact = await renderImpactWithMcp(preview.impact, codebase, root, changeId, requirement);
        if (unansweredBlockers.length > 0) {
            await writer.write(join(changeDirectory, "spec.json"), JSON.stringify({
                schemaVersion: "2.0.0",
                status: "CLARIFYING",
                requirement,
        ...
        if (!force) {
            let sameInput = false;
            try {
                const metadata = await writer.metadata(join(changeDirectory, "spec.md"));
                if (metadata === undefined)
                    throw new Error("缺少 spec 制品摘要");
        ...
        const impact = await renderImpactWithMcp(artifacts.impact, codebase, root, changeId, requirement);
        const proposal = parentChangeId === null
            ? artifacts.proposal
            : `${artifacts.proposal}\n\n## Based On Archived Change\n\n- ${parentChangeId}`;
