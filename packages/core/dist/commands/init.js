...
import { createEmptyProjectProfile, discoverProjectConventions, isEmptyProject, } from "../project-conventions/scanner.js";
import { ProjectConventionsStore } from "../project-conventions/store.js";
import { assertRecoverableCommandState, normalizeCommandError, persistCommandFailure, } from "./recovery.js";
...
export async function runInit(root, codebase, args, signal) {
    ...
    try {
        ...
        await writer.write(join(sddRoot, "config.yml"), stringify(defaultConfig(root, manifests.map((m) => m.agent))), { generatedBy: "sdd-harness", purpose: "config" });
        ...
        const index = await withTimeout(codebase.initialize(root), timeoutMilliseconds(args), "sdd init", signal);
        const indexInputs = {
        ...
        await writer.write(join(sddRoot, "index", "codebase-summary.md"), index.codebaseSummary, indexInputs);
        ...
        await conventionsStore.write(emptyProject
        ...
        const normalized = normalizeCommandError(error, "E_STATE_CORRUPTED", "sdd init");
        if (started) {
...
function defaultConfig(root, agentNames) {
    const plugins = {};
...
async function writeDependencyMetadata(sddRoot, codebaseProvider) {
    const entries = [
    ...
    await Promise.all(entries.map(async ([name, metadata]) => {
        ...
        await writeFile(join(directory, "version.json"), `${JSON.stringify({
            ...
            status: name === "codebase-memory-mcp"
...
async function verifyDependencyIntegrityIfProvided(sddRoot, args) {
...
function readIntegrityEvidence(args) {
    ...
    const codebaseMemoryMcp = integrity
        .codebaseMemoryMcp;
    if (codebaseMemoryMcp === undefined ||
        codebaseMemoryMcp === null ||
        typeof codebaseMemoryMcp !== "object") {
        return undefined;
    }
    const input = codebaseMemoryMcp;
    if (typeof input.manifestContent !== "string" ||
        typeof input.artifactContentBase64 !== "string") {
        throw new SddError("E_COMPONENT_INTEGRITY_FAILED", "组件完整性校验材料格式不正确");
    }
    return {
...
async function writeIntegrityReport(sddRoot, report) {
    const directory = join(sddRoot, "adapters", "codebase-memory-mcp");
    await mkdir(directory, { recursive: true });
    await writeFile(join(directory, "integrity.json"), `${JSON.stringify(report, null, 2)}\n`, "utf8");
}
function targetDescriptor(evidence) {
    return {
...
function buildWarnings(index, configWarnings) {
    const warnings = [];
    if (index.degraded) {
        warnings.push("降级模式：codebase-memory-mcp 当前不可用，已切换为受限文件扫描");
        warnings.push(`安装建议：请先安装并配置 codebase-memory-mcp，官方项目地址：${PINNED_DEPENDENCIES.codebaseMemoryMcp.repository}`);
    }
    warnings.push(...configWarnings);
    return warnings.length === 0 ? {} : { warnings };
}
function readStructurePolicy(args) {
 ...
 */
function normalizeAgentArg(raw) {
    if (raw === undefined || raw === null)
        return undefined;
    if (Array.isArray(raw))
        return raw.filter((a) => typeof a === "string" && a.length > 0);
 ...
 */
function filterManifests(all, selected) {
...
async function validateConfig(path) {
    ...
    try {
        ...
        throw new SddError("E_STATE_CORRUPTED", `config.yml 解析失败：${error instanceof Error ? error.message : String(error)}`, "sdd init");
    }
    const config = configSchema.safeParse(parsed);
    if (!config.success) {
        throw new SddError("E_STATE_CORRUPTED", `config.yml 校验失败：${config.error.issues.map((issue) => issue.path.join(".") || issue.message).join("; ")}`, "sdd init");
    }
    const unknownKeys = Object.keys(config.data).filter((key) => !REQUIRED_CONFIG_KEYS.includes(key));
    return unknownKeys.length === 0
        ? []
        : [`config.yml 包含未知字段，已保留原值：${unknownKeys.join(", ")}`];
}
async function migrateConfigIfNeeded(root, path) {
    ...
    await appendFile(join(root, ".sdd", "logs", "migration.log"), `${new Date().toISOString()} 1.0.0 -> 1.3.0 (config.yml)\n`, "utf8");
}
