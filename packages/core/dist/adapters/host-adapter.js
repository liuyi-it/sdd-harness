import { COMMANDS, ERROR_EXIT_CODES, } from "../contracts.js";
...
export class HostAdapter {
    ...
    async execute(input, cwd) {
        ...
        return renderResult(await this.core.execute({
...
export function parseHostCommand(input, cwd, style) {
    void cwd;
    ...
    const rawCommand = style === "claude-code"
        ? tokens.shift()?.replace(/^\/sdd\./, "")
        : parseCodexPrefix(tokens);
    if (rawCommand === undefined ||
        !COMMANDS.includes(rawCommand)) {
        throw new SddError("E_INVALID_PHASE_COMMAND", `未知的 sdd 命令：${rawCommand ?? ""}`);
    }
    const command = rawCommand;
    const args = {};
    // new/auto 允许第一个非选项参数直接作为自然语言需求输入。
    if ((command === "new" || command === "auto") &&
        ...
        switch (token) {
            ...
            case "--timeout": {
                ...
                if (!Number.isFinite(value) || value < 0)
                    throw new SddError("E_INVALID_PHASE_COMMAND", "--timeout 必须是非负数");
                args.timeout = value;
                break;
            }
            default:
                throw new SddError("E_INVALID_PHASE_COMMAND", `未知的选项：${token ?? ""}`);
        }
    }
    return { command, args, help, version: false };
}
function parseCodexPrefix(tokens) {
    if (tokens.shift() !== "sdd")
        return undefined;
    return tokens.shift();
}
...
function requireValue(tokens, option) {
...
function helpDocument(command, style) {
    ...
    return {
        ...
        examples: COMMAND_EXAMPLES[command].map((example) => style === "claude-code" ? example.replace(/^sdd /, "/sdd.") : example),
...
function textView(result) {
    ...
    if (result.warnings !== undefined && result.warnings.length > 0) {
        lines.push("", "Warnings:", ...result.warnings.map((item) => typeof item === "string"
            ? `- ${item}`
            : `- [${item.code}] ${item.message}`));
    }
    return `${lines.join("\n")}\n`;
}
function formatCommand(style, command) {
