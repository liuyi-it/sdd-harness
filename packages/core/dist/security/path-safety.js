...
export async function assertSafePath(root, candidate) {
    const rootPath = await realpath(root);
    if (isAbsolute(candidate) ||
        /^[A-Za-z]:[\\/]/.test(candidate) ||
        candidate.startsWith("\\\\")) {
        throw new SddError("E_PATH_OUTSIDE_REPO", `不允许使用绝对路径：${candidate}`);
    }
        ...
        try {
            ...
            if (stat.isSymbolicLink()) {
                ...
                if (!isInside(rootPath, target)) {
                    throw new SddError("E_SYMLINK_BLOCKED", `符号链接解析到仓库之外：${candidate}`);
                }
                cursor = target;
            }
        }
