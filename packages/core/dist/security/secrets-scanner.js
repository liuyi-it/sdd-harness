...
const UNCONDITIONAL_BLOCKED = new Set([
    "private-key",
    "github-token",
]);
const RULE_PATTERNS = {
    "aws-access-key": /\bAKIA[0-9A-Z]{16}\b/g,
    "github-token": /\b(?:gh[pousr]_[A-Za-z0-9]{36,255}|github_pat_[A-Za-z0-9_]{82})\b/g,
    jwt: /\beyJ[A-Za-z0-9_-]+\.eyJ[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+\b/g,
    "private-key": /-----BEGIN [A-Z ]*PRIVATE KEY-----[\s\S]*?-----END [A-Z ]*PRIVATE KEY-----/g,
    authorization: /\bAuthorization\s*[=:]\s*(?:Bearer|Basic|Token)\s+[A-Za-z0-9._~+/=-]{8,}/gi,
    "database-url": /\b(?:postgres|postgresql|mysql|mongodb|redis|amqp)(?:\+[a-z]+)?:\/\/[^\s:@]+:([^\s@]+)@[^\s/]+/gi,
    "generic-secret": /\b(?:token|password|passwd|secret|api[_-]?key|access[_-]?key)\s*[=:]\s*["']?([^\s"';<>]{6,})/gi,
};
const RULE_PREVIEWS = {
...
export function createSecretAllowList(rules = [], paths = []) {
    return {
        rules: new Set(rules),
        paths: new Set(paths),
    };
...
export function scanSecretFindings(text, file) {
    return scanSecrets({ text, ...(file === undefined ? {} : { file }) })
        .findings;
}
export function hasBlockingSecretFinding(findings) {
...
function redactedShape(value, rule) {
    switch (rule) {
        case "aws-access-key":
        case "github-token":
        case "jwt":
