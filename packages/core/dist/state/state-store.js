import { appendFile, cp, copyFile, mkdir, open, readFile, readdir, rename, stat, writeFile, } from "node:fs/promises";
...
import { CURRENT_SCHEMA_VERSION, migrateWorkflowState, } from "./schema-migration.js";
/**
...
const safeRelativePathSchema = z
    ...
    !value.includes("\\") &&
    !value.split("/").includes(".."), "必须是安全相对路径");
...
const activeLoopSchema = z
    ...
    if (loop.waiting?.reason === "AGENT_TASK_EXECUTION" &&
    ...
    if (["ABORTED", "SUCCEEDED", "ARCHIVED"].includes(loop.status) &&
...
export const workflowStateSchema = z
    ...
    artifacts: z.record(z.string(), z.enum(["MISSING", "READY", "CANDIDATE", "STALE"])),
    ...
    if (waiting?.reason === "AGENT_TASK_EXECUTION" &&
...
export class StateStore {
    ...
    async read() {
        try {
            ...
            if (raw.schemaVersion === "1.0.0") {
                ...
                await writeFile(`${this.path}.migration.bak`, `${JSON.stringify(raw, null, 2)}\n`, "utf8");
                await mkdir(join(this.root, ".sdd", "logs"), { recursive: true });
                await appendFile(join(this.root, ".sdd", "logs", "migration.log"), `${new Date().toISOString()} ${migration.from} -> ${migration.to}\n`, "utf8");
            ...
            return await this.normalizeActiveLoop(await this.normalizeTransientState(parsed));
        }
        catch (error) {
            ...
            try {
                // 优先回退到上一次完整写入时留下的备份。
                const backup = workflowStateSchema.parse(JSON.parse(await readFile(this.backupPath, "utf8")));
                ...
                if (inferred !== null)
                    return inferred;
                throw new SddError("E_STATE_CORRUPTED", `无法读取状态文件或备份：${error instanceof Error ? error.message : String(error)}`, "sdd status");
            }
        }
    }
    async write(state) {
    ...
    async validateChangeReference(state) {
        if (state.currentChangeId === null)
            return;
        try {
            if (!(await stat(join(this.root, ".sdd", "changes", state.currentChangeId))).isDirectory()) {
                throw new Error("not a directory");
            }
        }
        catch {
            throw new SddError("E_STATE_CORRUPTED", `currentChangeId 指向的变更不存在：${state.currentChangeId}`, "sdd status");
        }
    }
    async normalizeTransientState(state) {
        const recovery = TRANSIENT_PHASE_RECOVERY[state.currentPhase];
        if (recovery === undefined)
            return state;
        if (await pathExists(join(this.root, ".sdd", "lock")))
            return state;
        const normalized = workflowStateSchema.parse({
            ...
            tasks: state.currentPhase === "BUILDING"
                ? Object.fromEntries(Object.entries(state.tasks).map(([taskId, status]) => [
                    taskId,
    ...
    async recoverFromArtifacts() {
        ...
        if (changeId !== null) {
            ...
            if (await pathExists(join(change, ".archived")))
                ...
                phase = "DESIGN_READY";
            else if (await pathExists(join(change, "spec.md")))
                phase = "SPEC_READY";
        }
        const recovered = {
            ...createInitialState(),
        ...
        await writeFile(join(this.root, ".sdd", "state.recovered.json"), `${JSON.stringify(recovered, null, 2)}\n`, "utf8");
        return recovered;
    }
    async normalizeActiveLoop(state) {
        ...
        if (await new LoopStore(this.root).hasRun(loop.runId))
            return state;
...
async function reportPassed(path) {
    try {
...
function suggestedCommand(phase) {
    return ({
        INDEX_READY: "sdd new",
...
async function backupSddDirectory(root) {
...
const TRANSIENT_PHASE_RECOVERY = {
...
function migrateFrom120(raw) {
    const phase = raw.currentPhase;
    const suggested = raw.suggestedCommand;
