 ...
 * schemas 与适配器无关，始终安装。
 */
export async function installProjectIntegration(root, manifests, _options = {}) {
    const instructions = [
        ...new Map(manifests.map((manifest) => [
            manifest.instructionFile,
            manifest.instructionContent,
        ])).entries(),
    ];
    await Promise.all([
        ...instructions.map(([instructionFile, instructionContent]) => installInstructions(join(root, instructionFile), instructionContent, _options)),
...
async function installAdapterCapabilities(root, manifest) {
    const path = join(root, ".sdd", "adapters", manifest.agent, "capabilities.json");
 ...
 * force=true 时全量覆盖。
 */
async function installInstructions(path, managedContent, options) {
    ...
    // 追加新行到文件末尾
    const appendContent = `\n${newLines.join("\n")}`;
    await writer.write(path, existing + appendContent, managedInputs(managedContent));
}
/**
 * 按 manifest 的 commandsDir 和 commandTemplate 生成所有 sdd 命令文件。
 */
async function installCommands(root, manifest) {
    const directory = join(root, manifest.commandsDir);
    await mkdir(directory, { recursive: true });
    const writer = new ArtifactWriter();
    const inputs = managedInputs(manifest.commandTemplate);
    await Promise.all(COMMANDS.map((command) => writer.write(join(directory, `sdd.${command}.md`), manifest.commandTemplate.replaceAll("{command}", command), inputs)));
}
/**
 * 按 manifest 的 skillsDir 安装 SKILL.md。
 */
async function installSkill(root, manifest) {
    const skillPath = join(root, manifest.skillsDir, "SKILL.md");
    await mkdir(join(skillPath, ".."), { recursive: true });
    await new ArtifactWriter().write(skillPath, manifest.skillContent, managedInputs(manifest.skillContent));
}
/**
...
async function installSchemas(root) {
    ...
    await mkdir(directory, { recursive: true });
    const writer = new ArtifactWriter();
    await Promise.all(Object.entries(CANONICAL_SCHEMAS).map(async ([name, content]) => writer.write(join(directory, name), content, managedInputs(content))));
}
function managedInputs(content) {
    return {
        generatedBy: "sdd-harness",
        content,
    };
