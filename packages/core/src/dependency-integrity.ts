...
export function verifyChecksumManifest(
  input: VerifyChecksumManifestInput,
): void {
  ...
  if (manifestSha256 !== input.expectedManifestSha256) {
    throw new SddError(
      ...
      `校验清单摘要不匹配：期望 ${input.expectedManifestSha256}，实际 ${manifestSha256}`,
    );
  }
  const expectedArtifactSha256 = manifestEntry(
...
export function decodeArtifactContent(base64: string): Buffer {
