 ...
 */
export function wrapUntrustedRepositoryContent(
  content: string,
  file?: string,
): string {
  if (content.includes(REPOSITORY_BOUNDARY_END)) {
    throw new Error(
      `Untrusted content for ${file ?? "repository"} contains forbidden end marker; refusing to wrap.`,
    );
  }
 ...
 */
export function wrapUntrustedMcpOutput(
  content: string,
  intent?: string,
): string {
  if (content.includes(MCP_BOUNDARY_END)) {
    throw new Error(
      `MCP output for ${intent ?? "query"} contains forbidden end marker; refusing to wrap.`,
    );
  }
...
const ALLOWED_CONSTRAINT_KEYS: ReadonlySet<keyof TaskConstraints> = new Set([
  "allowedCommands",
  "allowedFiles",
  "expectedNewFiles",
  "forbiddenFiles",
...
export function buildTaskConstraints(input: TaskConstraints): TaskConstraints {
  ...
  if (input.rulesHash !== undefined) out.rulesHash = input.rulesHash;
  // 只保留白名单字段，丢弃任何额外属性。
  return Object.fromEntries(
    Object.entries(out).filter(([k]) =>
      ALLOWED_CONSTRAINT_KEYS.has(k as keyof TaskConstraints),
    ),
  ) as TaskConstraints;
}
 ...
 */
export function sanitizeAllowedCommands(values: readonly string[]): string[] {
...
function isAcceptableCommand(value: string): boolean {
