...
export async function readAuthoritativeSpec(
  change: string,
  markdown: string,
): Promise<{ document: SpecDocument; legacy: boolean }> {
  ...
  const scenarios = new Map(
    document.requirements.flatMap((requirement) =>
      requirement.scenarios.map(
        (scenario) => [scenario.id, requirement.id] as const,
      ),
    ),
  );
  for (const task of tasks) {
    ...
    for (const scenario of task.scenarios) {
      ...
      if (owner === undefined)
        ...
        failures.push(
          `${task.id} 的 Scenario ${scenario} 不属于其 Requirement`,
        );
    }
  }
    ...
    for (const scenario of requirement.scenarios) {
      ...
      for (const phase of PHASES) {
        ...
        if (phaseTasks.length === 0)
          ...
          failures.push(
            `${requirement.id}/${scenario.id} 的 ${phase} 任务重复`,
          );
        for (const task of phaseTasks) {
          ...
          const evidence =
            result.tddEvidence?.filter((entry) => entry.phase === phase) ?? [];
          ...
          if (
            ...
            !evidence.some(
              (entry) =>
                entry.passed === false && entry.expectedFailure === true,
            )
          )
          ...
          if (
            ...
            (!Array.isArray(result.verification) ||
              !result.verification.some(
                (entry) => entry.command.trim().length > 0 && entry.passed,
              ))
          )
...
export function renderTraceability(
  ...
  if (repairTasks.length > 0) {
    ...
    for (const task of repairTasks) {
      ...
      lines.push(
        `失败来源：${task.failureContext?.source ?? "unknown"}`,
        `错误码：${task.failureContext?.errorCode ?? "unknown"}`,
        `Review Findings：${task.failureContext?.findingIds?.join("、") || "无"}`,
        `前序 Loop Run：${task.failureContext?.previousRunId ?? "unknown"}`,
        `Policy：${task.policyRefs?.map((policy) => `${policy.id}@${policy.version} (${policy.digest})`).join("；") || "无"}`,
        `修改文件：${unique(result?.modifiedFiles ?? []).join("、")}`,
        `TDD 命令：${unique(result?.tddEvidence.map((entry) => entry.command) ?? []).join("；")}`,
        `验证命令：${unique(result?.verification.map((entry) => entry.command) ?? []).join("；")}`,
        "",
      );
    }
  }
...
function parseAndValidate(markdown: string): SpecDocument {
  ...
  try {
    ...
    throw corrupted(
      `spec.md 结构无效：${error instanceof Error ? error.message : String(error)}`,
    );
  }
...
function assertStableIds(document: SpecDocument): void {
  document.requirements.forEach((requirement, requirementIndex) => {
    ...
    if (requirement.id !== expected)
      throw corrupted(
        `spec.json requirements[${requirementIndex}].id 必须为 ${expected}`,
      );
    requirement.scenarios.forEach((scenario, scenarioIndex) => {
      ...
      if (scenario.id !== scenarioExpected)
        throw corrupted(
          `spec.json requirements[${requirementIndex}].scenarios[${scenarioIndex}].id 必须为 ${scenarioExpected}`,
        );
    });
  });
}
...
function assertValid(document: SpecDocument, name: string): void {
  ...
  if (failures.length > 0)
    throw corrupted(
      `${name} 校验失败：${failures.map((failure) => failure.message).join("；")}`,
    );
}
...
function isSpecDocument(value: unknown): value is SpecDocument {
  ...
  return value.requirements.every(
    (requirement) =>
      ...
      ["ADDED", "MODIFIED", "REMOVED"].includes(
        String(requirement.operation),
      ) &&
      Array.isArray(requirement.scenarios) &&
      requirement.scenarios.every(
        (scenario) =>
          isRecord(scenario) &&
          typeof scenario.id === "string" &&
          typeof scenario.title === "string" &&
          [scenario.given, scenario.when, scenario.then].every(
            (steps) =>
              Array.isArray(steps) &&
