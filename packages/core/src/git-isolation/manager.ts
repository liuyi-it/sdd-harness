...
export function normalizeGitIsolationConfig(
  config: GitIsolationConfig = {},
): NormalizedGitIsolationConfig {
...
export class GitIsolationManager {
  private readonly config: NormalizedGitIsolationConfig;
  ...
  async plan(changeId: string): Promise<ExecutionWorkspace> {
    ...
    const worktreePath = this.config.createWorktree
      ? await assertSafePath(
          this.controlRoot,
          join(this.config.worktreeDir, changeId),
        )
      : null;
    return {
  ...
  async ensure(changeId: string): Promise<ExecutionWorkspace> {
    const workspace = await this.plan(changeId);
    if (workspace.worktreePath === null || workspace.branchName === null) {
      return workspace;
    }
    const registered = (await this.git.worktreeList(this.controlRoot)).find(
      (entry) => entry.path === workspace.worktreePath,
    );
    ...
    if (exists !== (registered !== undefined)) {
      throw new SddError(
        "E_STATE_CORRUPTED",
        `worktree 路径与 Git 注册状态不一致：${workspace.worktreePath}`,
      );
    }
    ...
    if (registered?.branch !== workspace.branchName) {
      throw new SddError(
        "E_STATE_CORRUPTED",
        `worktree 分支不匹配：期望 ${workspace.branchName}，实际 ${registered?.branch ?? "detached"}`,
      );
    }
    if (registered.head !== workspace.baselineCommit) {
      throw new SddError(
        "E_STATE_CORRUPTED",
        `worktree HEAD 与基线不一致：期望 ${workspace.baselineCommit}，实际 ${registered.head}`,
      );
    }
    ...
    if (currentBranch !== workspace.branchName) {
      throw new SddError(
        "E_STATE_CORRUPTED",
        `worktree 当前分支不匹配：期望 ${workspace.branchName}，实际 ${currentBranch || "detached"}`,
      );
    }
    ...
    if (status.trim() !== "") {
      throw new SddError(
        "E_CONCURRENT_RUN",
        `worktree 存在未提交改动，拒绝复用：${workspace.worktreePath}`,
      );
    }
    return workspace;
  }
