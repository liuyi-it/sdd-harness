import type { SpecDocument, SpecValidationFailure } from "./model.js";

export function validateSpec(document: SpecDocument): SpecValidationFailure[] {
  const failures: SpecValidationFailure[] = [];
  const ids = new Set<string>();
  const operationsByTitle = new Map<string, string>();
...
function checkId(
  ...
  path: string,
  ids: Set<string>,
  failures: SpecValidationFailure[],
): void {
  if (ids.has(id)) {
    failures.push({
