...
export function parseSpec(markdown: string): SpecDocument {
  ...
  let requirement: SpecRequirement | undefined;
  let scenario: SpecScenario | undefined;
  ...
  for (let index = firstContent + 1; index < lines.length; index += 1) {
    ...
    if (stepMatch) {
      if (!scenario) throw new Error(`第 ${index + 1} 行存在孤立场景步骤`);
      const keyword = stepMatch[1]!.toLowerCase() as "given" | "when" | "then";
