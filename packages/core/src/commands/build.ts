...
function isValidV2Envelope(record: Record<string, unknown>): boolean {
  const fileDelta = record.fileDelta;
  const timestamps = record.timestamps;
  const mode = record.mode;
  return (
    VALID_TASK_STATUSES.includes(record.status as string) &&
...
function isValidCommandEvidence(value: unknown): boolean {
  if (!isRecord(value)) return false;
  return (
...
function isStringArrayRecord(value: unknown, keys: readonly string[]): boolean {
...
async function ensureFreshContextPacks(
  ...
  for (const task of tasks) {
    ...
    let metadata: ReturnType<typeof readContextPackMetadata> | undefined;
    try {
    ...
    const digestValid =
      contextPack !== undefined && verifyContextPackDigest(contextPack);
    if (
      metadata === undefined ||
      contextPack === undefined ||
      !digestValid ||
      ...
      await writeFile(
        ...
        renderContextPack({
          body:
            contextPack === undefined || !digestValid
...
function renderFallbackTaskBody(task: TaskDefinition): string {
  return [
    `# Task: ${task.id}`,
    "",
    `Phase: ${task.phase}`,
...
function invalidCompleteResult(message: string): CommandResult {
  return {
    ok: false,
    state: "FAILED",
    exitCode: 4,
...
function parseCompleteResult(
  ...
  if (typeof legacy !== "object" || legacy === null || Array.isArray(legacy))
    return invalidCompleteResult(
      "v2 task execution result 必须包含合法 legacy",
    );
  ...
  if (
    ...
    resultTaskId !== taskId ||
    !VALID_TASK_STATUSES.includes(status as string) ||
    ...
    !parsed.tddEvidence.every(
      (entry) =>
        ...
        typeof (entry as Record<string, unknown>).output === "string",
    ) ||
    !parsed.verification.every(
      (entry) =>
        typeof entry === "object" &&
        entry !== null &&
        !Array.isArray(entry) &&
...
async function buildCompleteTask(
  ...
  try {
    ...
    if (state.currentPhase !== "BUILD_WAITING_AGENT")
      return invalidCompleteResult(
        `无法在 ${state.currentPhase} 状态执行 build complete`,
      );
    const pending = state.pendingAgentTask;
    if (
      pending === null ||
    ...
    const taskStatus = "DONE" as const;
