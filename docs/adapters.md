...
- `sdd-spec`：复用已有决策，通过交互式选择澄清尚未明确的新需求或修订，再生成规格与技术设计；
...
用户要求完整实现时，五个阶段 Skill 在已有授权内顺序推进到交付；阶段切换不再机械确认。用户只要求分析、规格、计划或验证时遵守该范围。必要的新决策仍通过互斥选项询问，多任务目标未明和质量修复预算耗尽仍需用户决定。
