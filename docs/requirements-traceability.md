...
| 26.1-2 | CLI 可通过安装脚本全局安装（Windows Git Bash）          | `scripts/install.sh`                                        | `cli.test.ts`                                       |
...
| 26.2-4 | 两个平台的 plan 命令都能生成一致的 `plan.json`   | `commands/plan.ts`、两端 Adapter            | `design-plan.test.ts`、`acceptance.test.ts`                  |
