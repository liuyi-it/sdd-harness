...
1. Phase II-A：Schema 1.2.0、迁移、项目规范契约、Loop 持久化、TaskExecutor v2。
...
Adapter 只负责解析宿主参数、注入 TaskExecutor/MCP 能力和展示 `CommandResult`。Core 是 `.sdd/` 的唯一写入者，并新增七个职责边界：

- `schema`：Schema 1.2.0、真实制品校验和版本迁移。
- `project-conventions`：项目结构画像、规则解析和新鲜度校验。
...
Phase II-A 新增 `ProjectConventionProfile`，并持久化为：

```text
.sdd/project/conventions.json
.sdd/project/conventions.md
```

规范画像记录项目类型、结构策略、源码/测试/资源/配置/模块目录、命名与工具约定、规则文件路径和哈希、扫描证据、未知项、生成时间及代码库索引哈希。结构策略分为 `free-design`、`user-defined` 和 `discovered`。

init 必须区分空项目和已有项目。空项目进入 `CLARIFYING`，用户必须选择允许模型自由设计，或提供目录与技术规范；JSON/非交互调用未提供选择时不得默认自由设计。已有项目优先使用 codebase-memory-mcp 扫描结构和约定，不可用时降级到文件扫描。总结只能记录有路径证据的约定，无法确认的内容标记为 unknown，不得凭空推断。

每个 build 小任务开始前必须重新解析适用范围内的 `AGENTS.md` 和 `CLAUDE.md`。Codex 以 `AGENTS.md` 为当前宿主规则、`CLAUDE.md` 为补充；Claude Code 反之。支持根目录和任务文件所在目录的嵌套规则，范围更近的规则优先。无法安全解决的冲突进入 `CLARIFYING`。

Context Pack 必须包含规则来源、内容哈希、适用范围和明确的遵守声明；TaskExecutionRequest v2 同时携带规范版本和规则哈希，并把可机器判断的目录、文件和命令规则合并到 constraints。规则文件或项目结构画像在 plan 后发生变化时，必须重新生成 Context Pack。任务结果记录实际使用的规范版本与规则哈希，verify 据此检查规则是否已读取且未过期。

该机制只声明并验证规则加载、新鲜度和结构化约束，不宣称能够自动证明所有自然语言规则均被遵守。人工规范不得被重复 init 静默覆盖，重新生成必须显式请求。
...
- 单元测试：Schema、迁移、项目类型识别、规则解析、Loop、normalize、报告和安全规则。
- 集成测试：项目规范画像、规则新鲜度、state/loop-run 一致性、Git delta、MCP 降级和失败报告落盘。
...
Phase II-A 先建立 1.2.0 数据契约和迁移，再实现项目规范画像与逐任务规则注入，随后实现 Loop 和 TaskExecutor v2，使后续功能有稳定事实源和执行约束。Phase II-B 在此基础上接入 MCP、结构化质量报告和安全链路。Phase II-C 最后增加 Git 隔离，避免同时改变执行协议和工作目录语义。
