...
创建新 change 并返回统一规格阶段行动。需求最长 32768 个 Unicode 字符；可用全局 `--change` 指定新 ID。宿主先复用已有回答，对尚未明确且影响结果的目标、范围、验收和方案决策通过带选项的交互问题澄清，再回传结果；Core 只生成一个同时包含可验收需求、场景和技术设计的 `spec.md`。
