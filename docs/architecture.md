...
`verify` 检查 Requirement/Scenario、任务和证据覆盖；`review` 追加确定性审查、范围检查和敏感信息扫描。可恢复失败转换为 REPAIR 任务，超出预算或范围时进入 `PAUSED`。
...
`archive` 在同一写锁内重新验证报告摘要、Git 快照、文件范围、漂移和追踪闭环。成功后删除展开制品，变更目录只保留三个归档文件。`.archived` 保存组合哈希；状态更新中断时，再次执行 `archive` 会根据有效标记收敛到 `ARCHIVED`。
...
`vendor/openspec/upstream/` 和 `vendor/superpowers/upstream/` 是固定版本的审计快照，不作为外部 CLI 执行。运行时只复用其规格模型与任务方法语义，流程编排、状态、安全和质量门禁仍由 sdd-harness 实现。发布校验使用 `VERSION.json`、`MANIFEST.sha256` 和许可证信息验证快照完整性。
